...
class libraryfiles(db.Model):
  ...
  bookname = db.Column("bookname", EncryptedType(db.String(255), secretKey))
  filename = db.Column("filename", EncryptedType(db.String(255), secretKey))
  author = db.Column("author", EncryptedType(db.String(255), secretKey))
  category = db.Column("category", EncryptedType(db.String(75), secretKey))
...
def index():
  if "key" not in session:
    session["key"] = f"{uuid.uuid4()}"
    addkey = accesscode(session["key"])
...
def search():
  ...
  #POST requests should get and delete the users accesskey
  elif request.method == "POST":
    if "key" not in session:
      return "Missing Key"
    
    elif "key" in session:
      checkkey = accesscode.query.filter(accesscode.accesskey == session["key"]).first()
      if checkkey:
        db.session.delete(checkkey)
        session["key"] = f"{uuid.uuid4()}"
        addkey = accesscode(session["key"])
        db.session.add(addkey)
        db.session.commit()
        ...
        return render_template("accordion_result.html")
      elif not checkkey:
        return "Invalid Key"
...
@app.route("/test")
def testing():
  data = libraryfiles.query.filter(libraryfiles._id == 2).first()
  print(data.bookname)
  return 'hi'
